...
export const OnboardingModal: React.FC<OnboardingModalProps> = ({ onFinish }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/90 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in">
      <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-md w-full overflow-hidden shadow-2xl flex flex-col min-h-[500px] max-h-[90vh]">
        
        {/* Progress Bar */}
        <div className="flex gap-1 p-6 pb-0">
          {[0, 1, 2].map((idx) => (
             <div key={idx} className={`h-1.5 rounded-full flex-1 transition-all duration-300 ${idx <= currentStep ? 'bg-emerald-500' : 'bg-gray-200 dark:bg-slate-700'}`} />
          ))}
        </div>
        ...
        <div className="flex-grow p-6 flex flex-col items-center text-center justify-center">
           ...
           {currentStep === 0 && (
             <div className="animate-slide-up flex flex-col items-center">
               <div className="w-24 h-24 bg-emerald-100 dark:bg-emerald-900/30 rounded-full flex items-center justify-center mb-6 text-emerald-600 dark:text-emerald-400 shadow-emerald-200 dark:shadow-none shadow-lg">
           ...
           {currentStep === 1 && (
             <div className="animate-slide-up flex flex-col items-center">
               <div className="w-24 h-24 bg-blue-100 dark:bg-blue-900/30 rounded-full flex items-center justify-center mb-6 text-blue-600 dark:text-blue-400 shadow-blue-200 dark:shadow-none shadow-lg">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-12 h-12">
           ...
           {currentStep === 2 && (
             <div className="animate-slide-up flex flex-col items-center w-full">
               <div className="w-20 h-20 bg-amber-100 dark:bg-amber-900/30 rounded-full flex items-center justify-center mb-4 text-amber-600 dark:text-amber-400 shadow-amber-200 dark:shadow-none shadow-lg">
        ...
        <div className="p-6 pt-0 mt-auto">
          <button 
            ...
            className={`w-full py-4 rounded-xl font-bold text-lg transition-all active:scale-[0.98] shadow-lg flex items-center justify-center gap-2 ${
                ...
                : 'bg-gray-900 dark:bg-white text-white dark:text-black hover:bg-black dark:hover:bg-gray-200 shadow-gray-200 dark:shadow-none'
            }`}
          >
            {currentStep === 2 ? (
                <>
